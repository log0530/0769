...
foreach ($list as $coin) {
        ...
        $separate_aux = 1;
        echo "<tr style='background-color:#00CED1'><th colspan='8' style='text-align: left; color:#FFF;'>Merged Mining</th></tr>";
    }
    ...
    if (YAAMP_ALLOW_EXCHANGE && $coin->balance + $coin->mint < $owed * 0.9) {
        ...
        echo "<td><b><a href=\"/site/block?id={$coin->id}\" title=\"$title\" style=\"color: #c55;\">$name</a></b><span style=\"font-size: .8em;\"> ({$coin->algo})</span></td>";
         ...
         if ($coin->auxpow)
   echo "<td><b><a href='/site/block?id=$coin->id'><span style='color: red;'>$name</a></span></b><span style='font-size: .8em'> ($coin->algo)</span></td>";
    else
    ...
    if (!empty($coin->errors))
        echo "<td align=right style='font-size: .8em; color: #e60000' title='$coin->errors'>$height</td>";
    else
    ...
    if ($coin->auxpow && $coin->auto_ready)
        echo "<td align=right style='font-size: .8em; opacity: 0.6;color: #b33984;' title='merge mined\n$network_hash' data='$pool_hash_pow'>$pool_hash_pow_sfx</td>";
    else
