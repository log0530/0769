...
for($i = $start; $i > max(1, $start-101); $i--)
{
...
$pager = '';
if ($start <= $coin->block_height - 100)
	$pager  = $coin->createExplorerLink('<< Prev', array('start'=>min($coin->block_height,$start+100)));
if ($start != $coin->block_height)
	$pager .= '&nbsp; '.$coin->createExplorerLink('Now');
if ($start > 100)
	$pager .= '&nbsp; '.$coin->createExplorerLink('Next >>', array('start'=>max(1,$start-100)));
...
<input type="text" name="txid" class="main-text-input" placeholder="Transaction hash" style="width: 250px; margin: 4px;">
